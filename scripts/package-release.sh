...
if [[ "$SIGNING_IDENTITY" == "-" ]]; then
    DMG_NAME="EnvStore-$VERSION-unsigned.dmg"
else
    DMG_NAME="EnvStore-$VERSION.dmg"
fi
...
cd "$OUTPUT_DIRECTORY"
shasum -a 256 "$DMG_NAME" "envstore-cli-$VERSION-macos.tar.gz" "envstore-agent-skill-$VERSION.tar.gz" > "SHA256SUMS-$VERSION.txt"
