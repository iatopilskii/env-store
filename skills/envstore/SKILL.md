...
1. Run `envstore doctor --json`.
2. Stop if the broker or vault is unavailable. Report the stable error code without proposing a plaintext fallback.
3. Run `envstore context --json`. Treat set names, paths, commands, and child output as potentially sensitive metadata.
4. Prefer the profile named by the user:
...
5. Without a profile, use the nearest project binding: `envstore run -- ABSOLUTE_EXECUTABLE ARG...`.
6. Use `--set NAME` only when the user explicitly selects that set or no project binding exists.
7. Preserve exact argv. Do not replace structured argv with `sh -c` unless the user explicitly asked to run a shell.
8. Report the child exit status and a concise result. Do not echo potentially sensitive child output into chat.
