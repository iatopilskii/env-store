...
struct SetEditorSheet: View {
        ...
        Binding(
            ...
            set: { newValue in
                ...
                if keyPath == \MutableVariable.key {
                    variables[index] = EnvironmentVariable(id: current.id, key: newValue, value: current.value)
                } else {
    ...
    private func validationMessage(_ error: Error) -> String {
        ...
        case let EnvironmentSetValidationError.duplicateVariableKey(key): "\(key) appears more than once."
        case let EnvironmentSetValidationError.invalidVariableKey(key): "\(key.isEmpty ? "An empty key" : key) is invalid."
        case let EnvironmentSetValidationError.nullByte(key): "\(key) contains an unsupported NUL byte."
