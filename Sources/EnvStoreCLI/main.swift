...
enum EnvStoreCLI {
    ...
    private static func execute(arguments: [String]) throws -> Int32 {
        ...
        case "run":
            ...
            writeError("EnvStore requests access to '\(payload.setName)' for \(displayedCommand(payload)).\n")
            response = try transport.send(BrokerRequest(operation: .run, run: payload))
        case "grant":
    ...
    private static func executeGrant(
        ...
        case "request":
            ...
            writeError(
                "EnvStore requests a grant for '\(command.setName)': \(displayedCommand(command)); ttl \(Int(ttl))s; uses \(uses).\n"
            )
    ...
    private static func parseRun(_ arguments: [String]) throws -> RunCommandPayload {
        guard let setName = try option("--set", in: arguments), !setName.isEmpty else {
            throw CLIError.invalidArguments("--set NAME is required")
        }
    ...
    private static func parseDuration(_ value: String) throws -> TimeInterval {
        ...
        case "h": multiplier = 3_600; number = String(value.dropLast())
    ...
    private static func printHuman(_ response: BrokerResponse, operation: String) {
        ...
        if let grants = response.grants {
            for grant in grants {
                print("\(grant.id.uuidString)  \(grant.setName)  uses=\(grant.remainingUses)  expires=\(grant.expiresAt.formatted(.iso8601))")
            }
