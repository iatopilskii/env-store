...
public struct DotenvParser: Sendable {
    ...
    private func parseUnquotedValue(_ rawValue: String) -> String {
        ...
        for index in characters.indices where characters[index] == "#" {
            guard index > characters.startIndex, characters[characters.index(before: index)].isHorizontalWhitespace else {
                continue
            }
    ...
    private func appendUnquotedTail(after quoteIndex: String.Index, in segment: String, to value: inout String) {
...
private struct ParseAccumulator {
    ...
    mutating func addVariable(key: String, value: String, sourceLine: Int) {
        if variablesByKey[key] != nil {
            issues.append(DotenvIssue(severity: .warning, code: .duplicateKey, line: sourceLine, key: key))
        } else {
