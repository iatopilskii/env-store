...
public final class VaultViewModel: ObservableObject {
    ...
    public func unlock() async {
        ...
        do {
            ...
            selectedSetID = selectedSetID.flatMap { id in
    ...
    public func saveProjectBinding(path: String, setID: UUID) async -> Bool {
        ...
        let binding = projectBindings.first(where: { $0.path == normalized })
    ...
    private func friendlyMessage(for error: Error) -> String {
        ...
        case let EnvironmentSetValidationError.duplicateVariableKey(key):
            "The key \(key) appears more than once."
        case let EnvironmentSetValidationError.invalidVariableKey(key):
            "The key \(key) is not a valid environment variable name."
        case let EnvironmentSetValidationError.nullByte(key):
